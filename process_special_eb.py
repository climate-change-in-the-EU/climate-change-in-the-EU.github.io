...
dir_original_eb = '../climatechange/original_data/special_eb/'
dir_processed_eb = '../climatechange/processed_data/special_eb/'
...
def process_special_eb():
    ...
    for special_eb_file in os.listdir(dir_original_eb):
        ...
        # If your spreadsheet is very large, you can add an argument on_demand, which loads only current sheets to memory:
        workbook = xlrd.open_workbook('my_file_name.xls', on_demand = True)'''
        # Go through each sheet of the workbook
        for sheet_name in workbook.sheet_names():
            sheet = workbook.sheet_by_name(sheet_name)
            # Filter out a sheet if it contains one of the specified questions
